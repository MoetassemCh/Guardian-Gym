...
const addCategory = async (req, res) => {
    try {
        ...
        try {
            ...
            else{ res.status(409).json({message: "Failed to Add Category"}); }
...
const patchExercise = async (req, res) => {
    ...
    try{
        ...
        if(result.matchedCount !== 1){ res.status(400).json( { message : "Category Does Not Exist" } ) }

        if(result.modifiedCount === 1){ res.status(200).json( { message : "Category Updated!" } ) }

        else { res.status(200).json({ message : "Category Updated!", Category}) }
      }
